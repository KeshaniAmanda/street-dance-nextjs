...
export default function Hero() {
  ...
  // 🔥 LINKS
  const PURCHASE_LINK = "https://themeforest.net/checkout/114152339/create_account";
  ...
  // ✅ UPDATED SIGNUP FUNCTION (NEW TAB → /signup route)
  const handleSignup = () => {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden">
      ...
      {slides.map((slide, index) => (
        <div
          ...
          className={`absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ${
            index === current ? "opacity-100 z-10 hero-bg" : "opacity-0 z-0"
          }`}
      ...
      >
        {slides[current].id === 2 ? (
          <div className="w-full flex flex-col md:flex-row items-center justify-between gap-10 text-white">
            ...
            {/* LEFT TITLE */}
            <div className="md:w-1/2 text-right ml-auto">
              <div className="hero-mask">
                <h1 className={`${titleClass} hero-title`}>
            ...
            <div className="md:w-1/2">
              <p className="hero-desc text-[16px] md:text-[17px] text-gray-300 mb-10 leading-relaxed max-w-md">
                {slides[current].desc}
              </p>
              ...
              <div className="flex gap-4 flex-wrap">
                ...
                {slides[current].buttons.includes("signup") && (
                  <button
                    ...
                    className="hero-btn bg-black text-white px-8 py-3 text-sm font-semibold uppercase tracking-[0.2em] border-none hover:bg-white hover:text-black transition-all duration-300"
                  >
                    Sign Up
                  </button>
                )}
                ...
                {slides[current].buttons.includes("purchase") && (
                  <button
                    ...
                    className="hero-btn bg-yellow-400 text-black px-8 py-3 text-sm font-semibold uppercase tracking-[0.2em] border-2 border-yellow-400 hover:bg-transparent hover:text-yellow-400 transition duration-300"
                  >
                    Purchase
                  </button>
                )}
          ...
          <div className="text-white max-w-3xl">
            ...
            <p
              className={`hero-desc text-[16px] md:text-[17px] text-gray-300 mb-10 leading-relaxed max-w-md ${
      ...
      <button
        onClick={prevSlide}
        className="absolute left-6 top-1/2 -translate-y-1/2 text-white text-8xl z-40 hover:text-yellow-400 transition"
      >
        ‹
      </button>
      ...
      <button
        onClick={nextSlide}
        className="absolute right-6 top-1/2 -translate-y-1/2 text-white text-8xl z-40 hover:text-yellow-400 transition"
      >
        ›
      </button>
    </div>
